  ...
  for(i in 1:trials) {
    ...
    if (i>=3) {
      if (choice[i-1] == choice[i-2] & feedback[i-1] == 1 & feedback[i-2] == 1) {
          ...
          choice[i] <- rbinom(1,1,inv_logit_scaled(logit_rate+betagamble*gamble[i])) # If both of them were success, switch
        } else if (choice[i-1] == choice[i-2] & feedback[i-1] == 0 & feedback[i-2] == 0) {
          gamble[i] <- (-1+2*choice[i-1])
          choice[i] <- rbinom(1,1,inv_logit_scaled(logit_rate+betagamble*gamble[i])) # If both of them were fail, stay
        } else { # If feedback changed
          choice[i] <- RandomAgentNoise_f(rate, noise) # Randomly generate based on rate and noise
        }
    } else { # If it hasn't been 3 trials yet
...
for (noiseLvl in unique(d$noise)) {
  ...
  for (rateLvl in unique(d$rate)) {
    # Generate list of choices
    outcome <- WinLoseResults_model_f(trials, noiseLvl, rateLvl, betagamble = 10)
...
write_csv(recovery_df, "simdata/W3_recoverydf_alternative.csv")
```

```{r visualising pr}
recovery_df <- read_csv("simdata/W3_recoverydf_alternative.csv")
