...
// generated quantities{
//   real rate_prior;  // theta prior parameter, on a prob scale (0-1)
//   real rate_posterior; // theta posterior parameter, on a prob scale (0-1)
//   real betaGamble_prior;  // theta prior parameter, on a prob scale (0-1)
//   real betaGamble_posterior; // theta posterior parameter, on a prob scale (0-1)
...
//   rate_posterior = rate;  // converting the posterior estimate from log odds to prob.
//   betaGamble_prior = normal_rng(0,1); // generating the prior on a log odds scale and converting
//   betaGamble_posterior = betaGamble;  // converting the posterior estimate from log odds to prob.
//   prior_preds = binomial_rng(trials, inv_logit(rate_prior));
//   // gamble_mean = mean(gamble);
//   // posterior_preds = binomial_rng(trials, inv_logit(rate_posterior+betaGamble_posterior*mean(gamble)));
//   posterior_preds = binomial_rng(trials, inv_logit(rate_posterior));
// }
